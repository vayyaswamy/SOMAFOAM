...
)
{
    ...
    forAll (matrices, rowI)
    {
        ...
        //Info << "curMatrix Initial " << curMatrix.diag() << endl;

        //Info << "upper " << curMatrix.upper() << endl;
        ...
        //Info << "PsiRef = " << psiRef << endl;

        psi.set(rowI, &psiRef);
        source.set(rowI, new scalarField(curMatrix.source()));

        //Info << "source = " << curMatrix.source() << endl;

        //Info << "boundary coeffs " << curMatrix.boundaryCoeffs() << endl;

        //Info << "internal coeffs " << curMatrix.internalCoeffs() << endl;

        curMatrix.addBoundarySource(source[rowI], 0);

        interfaces[rowI] = curMatrix.psi().boundaryField().interfaces();

        //Info << "curMatrix after addBoundarySource " << curMatrix.diag() << endl;

        curMatrix.addBoundaryDiag(curMatrix.diag(), 0);

        //Info << "curMatrix after addBoundaryDiag " << curMatrix.diag() << endl;
    ...
    forAll (matrices, rowI)
    {
        ...
        //Info << "inside loop " << endl;

        //Info << curMatrix.diag() << endl;

        //Info << "psiRef = " << psiRef.internalField();
    ...
    //Info << "correctBoundaryConditions done" << endl;
