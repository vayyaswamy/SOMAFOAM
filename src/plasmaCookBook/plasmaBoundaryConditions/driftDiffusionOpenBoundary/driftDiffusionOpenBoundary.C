...
#include "driftDiffusionOpenBoundary.H"
...
Foam::driftDiffusionOpenBoundary::driftDiffusionOpenBoundary
(
    const driftDiffusionOpenBoundary& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::driftDiffusionOpenBoundary::updateCoeffs()
{
    if (this->updated())
    {
        return;
   ...
   // const fvPatchField<vector>& Fif=
    ...
    const scalarField C2 = Df*this->patch().deltaCoeffs();
