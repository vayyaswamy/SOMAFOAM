...
bool Foam::pimpleControl::criteriaSatisfied()
{
    ...
    //Info << "inside pimpleControl 1" << endl;
    ...
    //Info << "mesh_solutionDict " << mesh_.solutionDict() << endl;
    forAllConstIter(dictionary, solverDict, iter)
    {
        const word& variableName = iter().keyword();
        const label fieldI = applyToField(variableName);

        //Info << "inside pimpleControl 2" << endl;
        //Info << "variableName " << variableName << endl;
        //Info << "fieldI " << fieldI << endl;
        if (fieldI != -1)
        {
            scalar residual = 0;
