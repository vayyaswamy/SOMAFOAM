...
void Foam::driftDiffusionElectronDensity::updateCoeffs()
{
    ...
    forAll(temp1, facei)
    {
		...
		temp1[facei]=(0.25*vth  + (a-1)*muef[faceCelli]*(Ef[faceCelli]&n[facei])-0*Def[facei]*this->patch().deltaCoeffs()[facei]);
        temp2[facei]=a*seec_*(Fif[faceCelli]&n[facei])*this->patch().deltaCoeffs()[facei];
        temp3[facei]= 0*Def[facei]*this->patch().deltaCoeffs()[facei];
	}
    //Info << "lets see the value of muef" << muef->patchInternalField() << endl;
    this->refValue() = 0;
    this->valueFraction() = 1-temp1/(-mob_bound*(Ef_bound&n)-temp3+SMALL); 
