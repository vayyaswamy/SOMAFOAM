...
void Foam::driftDiffusionPositiveIonDensity::updateCoeffs()
{
    ...
    const fvPatchField<scalar>& muf=
        patch().lookupPatchField<volScalarField, scalar>("mobility" + fieldName_);

    const fvPatchField<scalar>& Df=
        patch().lookupPatchField<volScalarField, scalar>("diffusion" + fieldName_);
   ...
   // const fvPatchField<vector>& Fif=
    ...
    this->valueFraction() = (1-a)*C1/(C1+C2*this->patch().deltaCoeffs()+1e-50);
