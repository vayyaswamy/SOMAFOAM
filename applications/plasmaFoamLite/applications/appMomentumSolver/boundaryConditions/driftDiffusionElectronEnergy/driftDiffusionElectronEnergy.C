...
void Foam::driftDiffusionElectronEnergy::updateCoeffs()
{
 ...
 vectorField n = patch().nf();//normal vector
    ...
    const fvPatchField<scalar>& Te_bound=
    ...
    forAll(temp1, facei)
    {
        Info << "yaha BC samma aayo" << endl;
        label faceCelli = patch().faceCells()[facei];
        int a = pos(Ef[facei]&n[facei]);
        float vth = sqrt(8.0*1.38e-23*Te_bound[facei]/9.1e-31/acos(-1.0));
        temp1[facei]=(0.25*vth+(a-1)*muef[facei]*(Ef[facei]&n[facei])-0*Def[facei]*this->patch().deltaCoeffs()[facei]);
        temp2[facei]=(9/10)*1.38e-23*Tse_*a*seec_*(Fif[facei]&n[facei])*this->patch().deltaCoeffs()[facei];
        temp3[facei]= -muef[facei]*(Ef_bound[facei]&n[facei])-0*Def[facei]*this->patch().deltaCoeffs()[facei];
 	}
    this->refValue() = 0;
    this->valueFraction() = 1-temp1/(temp3+1e-38);
    ...
    label faceCelli = patch().faceCells()[facei];
        ...
        temp1[facei]=(0.25*vth  + (a-1)*(5/3)*muef[facei]*(Ef[facei]&n[facei])-(5/3)*Def[facei]*this->patch().deltaCoeffs()[facei]);
        temp2[facei]=(5/2)*1.38e-23*Tse_*a*seec_*(Fif[facei]&n[facei])*this->patch().deltaCoeffs()[facei];
        temp3[facei]= (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];
 	}
    this->refValue() = 0;
    this->valueFraction() = 1-temp1/(((5/3)*vel_bound&n)-temp3+1e-38);
    this->refGrad() = temp2/(temp1+1e-38);
    mixedFvPatchField<scalar>::updateCoeffs();*/
}
