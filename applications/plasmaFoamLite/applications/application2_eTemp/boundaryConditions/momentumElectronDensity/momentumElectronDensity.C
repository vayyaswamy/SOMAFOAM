...
void Foam::momentumElectronDensity::updateCoeffs()
{
    ...
    const volScalarField& velocityCell =
        db().objectRegistry::lookupObject<volScalarField>("velocityelectron");
    ...
    forAll(temp1, facei)
    {
        ...
        int a = pos(velocityCell[faceCelli]&n[facei]);
        float vth = sqrt(8.0*1.38e-23*Tef[faceCelli]/9.1e-31/acos(-1.0)) ;
		temp1[facei]=(0.25*vth  + a*(velocityCell[faceCelli]&n[facei])-Def[faceCelli]*this->patch().deltaCoeffs()[facei]);
        temp2[facei]=a*seec_*(Fif[faceCelli]&n[facei])*this->patch().deltaCoeffs()[facei];
        temp3[facei]= Def[faceCelli]*this->patch().deltaCoeffs()[facei];
	}
    //Info << "lets see the value of muef" << muef->patchInternalField() << endl;
    this->refValue() = 0;
    this->valueFraction() = 1-temp1/((vel_bound&n)-temp3+SMALL); 
    this->refGrad() = temp2/(temp1+SMALL);
