...
void Foam::momentumElectronEnergy::updateCoeffs()
{
 ...
 vectorField n = patch().nf();//normal vector
    ...
    forAll(temp1, facei)
    {
		...
		temp1[facei]=(0.25*vth  + a*(velocityCell[faceCelli]&n[facei])-(5/3)*Def[faceCelli]*this->patch().deltaCoeffs()[facei]);
        temp2[facei]=(5/2)*1.38e-23*Tse_*a*seec_*(Fif[faceCelli]&n[facei])*this->patch().deltaCoeffs()[facei];
        temp3[facei]= (5/3)*Def[faceCelli]*this->patch().deltaCoeffs()[facei];
	}
    this->refValue() = 0;
