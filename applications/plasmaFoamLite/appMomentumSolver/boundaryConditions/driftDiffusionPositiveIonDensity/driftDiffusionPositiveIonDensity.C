...
:
    mixedFvPatchField<scalar>(p, iF),
    fieldName_(iF.name()),
    vTherm_(readScalar(dict.lookup("vTherm")))
{
    this->refValue() = 0.0;
    this->refGrad() = 0.0;
...
void Foam::driftDiffusionPositiveIonDensity::updateCoeffs()
{
    ...
    const volScalarField& muef =
        db().objectRegistry::lookupObject<volScalarField>("mobility"+ fieldName_);
    ...
    forAll(temp1, facei)
    {
        ...
        int a = pos(mag(Ef[facei]&n[facei]));
...
void Foam::driftDiffusionPositiveIonDensity::write(Ostream& os) const
{
    ...
    os.writeKeyword("vTherm")
        << vTherm_ << token::END_STATEMENT << nl;
