...
\*---------------------------------------------------------------------------*/
#include "electronDensity_wilsonBC5.H"
...
Foam::electronDensity_wilsonBC5::electronDensity_wilsonBC5
(
    const fvPatch& p,
...
Foam::electronDensity_wilsonBC5::electronDensity_wilsonBC5
(
    const electronDensity_wilsonBC5& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::electronDensity_wilsonBC5::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    const volVectorField& gradne =
        db().objectRegistry::lookupObject<volVectorField>("gradne");
    ...
    forAll(expterm, facei)
    {
		...
		expterm[facei]= a*seec_*(Fif[faceCelli]&n[facei]) + 0.5*Def[facei]*(gradne[faceCelli]&n[facei]);//0.5*vth*(1-a)*seec_*(Fif[facei]&n[facei])/(muef[facei]*(Ef_bound[facei]&n[facei]))+2*(1-a)*seec_*(Fif[facei]&n[facei]);
...
void Foam::electronDensity_wilsonBC5::write(Ostream& os) const
{
...
// * * * * * * * * * * * * * * * Member Operators  * * * * * * * * * * * * * //
void Foam::electronDensity_wilsonBC5::operator=
(
