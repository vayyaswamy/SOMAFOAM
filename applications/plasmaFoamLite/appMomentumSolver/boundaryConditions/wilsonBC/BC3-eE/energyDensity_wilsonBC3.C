...
\*---------------------------------------------------------------------------*/
#include "energyDensity_wilsonBC3.H"
...
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
Foam::energyDensity_wilsonBC3::energyDensity_wilsonBC3
(
    const fvPatch& p,
...
Foam::energyDensity_wilsonBC3::energyDensity_wilsonBC3
(
    const energyDensity_wilsonBC3& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::energyDensity_wilsonBC3::updateCoeffs()
{
   if (this->updated())
    {
        return;
    ...
    forAll(pp, facei)
    {
		...
		//expterm[facei]=a*(1.38e-23*Tse_*seec_)*(Fif[faceCelli]&n[facei]);// (2/3)*vth*(1-a)*(1.38e-23*Tse_*seec_)*(Fif[facei]&n[facei])/(muef[facei]*(Ef_bound[facei]&n[facei]))+ 2*(1-a)*(1.38e-23*Tse_*seec_)*(Fif[facei]&n[facei]);
...
void Foam::energyDensity_wilsonBC3::write(Ostream& os) const
{
...
void Foam::energyDensity_wilsonBC3::operator=
(
