...
void Foam::momentumElectronDensity::updateCoeffs()
{
    ...
    forAll(expterm, facei)
    {
        ...
        pp[facei] = (1-a)*(vef[faceCelli]&n[facei]) + 0.5*vth;//- Def[facei]*this->patch().deltaCoeffs()[facei];//(1-2*a)*muef[facei]*(Ef[facei]&n[facei]) + 0.5*vth- Def[facei]*this->patch().deltaCoeffs()[facei];
        qq[facei] = (vel_bound[facei]&n[facei]) ;// - Def[facei]*this->patch().deltaCoeffs()[facei];
        //int a = pos(-Ef[faceCelli]&n[facei]);
        //float vth = sqrt(8.0*1.38e-23*Tef[faceCelli]/9.1e-31/acos(-1.0)) ;
