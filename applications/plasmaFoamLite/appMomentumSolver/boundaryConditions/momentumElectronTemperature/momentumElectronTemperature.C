...
\*---------------------------------------------------------------------------*/
#include "momentumElectronTemperature.H"
...
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
Foam::momentumElectronTemperature::momentumElectronTemperature
(
    const fvPatch& p,
...
Foam::momentumElectronTemperature::momentumElectronTemperature
(
    const momentumElectronTemperature& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::momentumElectronTemperature::updateCoeffs()
{
  if (this->updated())
    {
        return;
    ...
    const fvPatchField<scalar>& electron_bound=
        patch().lookupPatchField<volScalarField, scalar>("electron");
    const volVectorField& gradeD =
        db().objectRegistry::lookupObject<volVectorField>("gradeD");
    ...
    forAll(expterm, facei)
    {
		...
		expterm[facei]=a*(1.38e-23*Tse_*seec_)*(Fif[faceCelli]&n[facei]) ;//+ (5/6)*Def[facei]*(gradeD[faceCelli] & n[facei]);// (2/3)*vth*(1-a)*(1.38e-23*Tse_*seec_)*(Fif[facei]&n[facei])/(muef[facei]*(Ef_bound[facei]&n[facei]))+ 2*(1-a)*(1.38e-23*Tse_*seec_)*(Fif[facei]&n[facei]);
        pp[facei] = (1-a)*(5/2)*(vef[faceCelli]&n[facei])*electron[faceCelli]*1.38e-23 + vth*1.38e-23*electron[faceCelli] - Def[facei]*this->patch().deltaCoeffs()[facei];//(1-2*a)*(5/3)*muef[facei]*(Ef[facei]&n[facei]) + (2/3)*vth- (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];
        qq[facei] = (5/2)*(Vef_bound[facei]&n[facei])*1.38e-23*electron_bound[facei] - Def[facei]*this->patch().deltaCoeffs()[facei];
        //int a = pos(-Ef[faceCelli]&n[facei]);
        //float vth = sqrt(8.0*1.38e-23*Tef[faceCelli]/9.1e-31/acos(-1.0)) ;
        ...
        //pp[facei] =(1-2*a)*(5/3)*muef[facei]*(Ef[faceCelli]&n[facei]) + (2/3)*vth- (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];
        //qq[facei] = -(5/3)*muef[facei]*(Ef_bound[faceCelli]&n[facei]) - (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];
	}
...
void Foam::momentumElectronTemperature::write(Ostream& os) const
{
...
void Foam::momentumElectronTemperature::operator=
(
