...
\*---------------------------------------------------------------------------*/
#include "electronDensity_wilsonBC2.H"
...
Foam::electronDensity_wilsonBC2::electronDensity_wilsonBC2
(
    const fvPatch& p,
...
Foam::electronDensity_wilsonBC2::electronDensity_wilsonBC2
(
    const electronDensity_wilsonBC2& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::electronDensity_wilsonBC2::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    forAll(pp, facei)
    {
        ...
        pp[facei] = (a-1)*muef[facei]*(Ef[faceCelli]&n[facei])- Def[facei]*this->patch().deltaCoeffs()[facei];//(1-2*a)*muef[facei]*(Ef[facei]&n[facei]) + 0.5*vth- Def[facei]*this->patch().deltaCoeffs()[facei];
...
void Foam::electronDensity_wilsonBC2::write(Ostream& os) const
{
...
// * * * * * * * * * * * * * * * Member Operators  * * * * * * * * * * * * * //
void Foam::electronDensity_wilsonBC2::operator=
(
