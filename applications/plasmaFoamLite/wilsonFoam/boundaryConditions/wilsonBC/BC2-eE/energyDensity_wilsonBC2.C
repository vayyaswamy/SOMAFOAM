...
\*---------------------------------------------------------------------------*/
#include "energyDensity_wilsonBC2.H"
...
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
Foam::energyDensity_wilsonBC2::energyDensity_wilsonBC2
(
    const fvPatch& p,
...
Foam::energyDensity_wilsonBC2::energyDensity_wilsonBC2
(
    const energyDensity_wilsonBC2& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::energyDensity_wilsonBC2::updateCoeffs()
{
   if (this->updated())
    {
        return;
    ...
    forAll(pp, facei)
    {
        ...
        pp[facei] = (a-1)*(5/3)*muef[facei]*(Ef[faceCelli]&n[facei]) - (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];//(1-2*a)*(5/3)*muef[facei]*(Ef[facei]&n[facei]) + (2/3)*vth- (5/3)*Def[facei]*this->patch().deltaCoeffs()[facei];
...
void Foam::energyDensity_wilsonBC2::write(Ostream& os) const
{
...
void Foam::energyDensity_wilsonBC2::operator=
(
